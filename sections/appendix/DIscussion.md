...
멤버 변수들은 항상 클래스의 정의부에서 선언된 순서에 따라 초기화되므로, 생성자의 초기화리스트에 그 순서대로 작성해야 한다.
선언과 다른 순서로 작성하는 것은 의미를 알 수 없는 코드를 만드는데, 멤버 변수들이 눈에 보이는 순서대로 실행되지 않아 순서에 종속적인 버그를 찾기 어렵기 때문이다.
